...
1. 新專案先執行 `supabase/migrations/002_community_board.sql`；既有留言板執行 `004_simplify_community_category.sql` 後，再執行 `005_repair_community_read.sql`。
2. 檢查三張 table、兩個 public view、trigger、service-role grants 與 index。
...
- 前端只呼叫 Netlify Functions；read Function 優先讀 public views，View 缺失時只從 base tables 選取 visible 公開欄位。
...
若頁面顯示「留言板目前無法載入」，先在同一個 Supabase project 執行：
...
- 確認三張 Community base tables 已存在。
- 重建兩個只包含公開欄位的 View。
- 明確授權 Netlify Functions 使用的 `service_role`。
- 保持 `anon` 與 `authenticated` 無法直接讀寫 base tables。
...
然後觸發一次新的 Netlify deploy。只執行 SQL、不重新部署 Functions，舊版 `community-read.js` 不會取得 fallback 行為。
