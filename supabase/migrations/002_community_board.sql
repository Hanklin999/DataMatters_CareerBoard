...
create unique index if not exists community_reports_one_per_fingerprint_idx
  on public.community_reports(target_type, target_id, fingerprint_hash)
...
-- The browser has no direct base-table or view access. Public reads and all writes
-- go through Netlify Functions. The service role is server-only and the views below
-- limit community-read responses to visible, non-moderation fields.
