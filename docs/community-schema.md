...
For an existing deployment, run `supabase/migrations/005_repair_community_read.sql` after the base schema. It recreates the public-field views, grants the server-only `service_role`, and reloads the PostgREST schema cache. `community-read` also falls back to explicit visible base-table columns when the views are temporarily unavailable.
