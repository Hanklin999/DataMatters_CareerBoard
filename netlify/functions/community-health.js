...
exports.handler=async(event)=>{
  ...
  if(!config.configured){
    return json(503,{ok:false,message:"server_not_configured",missing:config.missing,invalid:config.invalid,url_source:config.urlSource,key_source:config.keySource,salt_source:config.saltSource});
  }
  try{
    await supabase("community_posts?select=id&limit=1");
    return json(200,{ok:true,database:true,url_source:config.urlSource,key_source:config.keySource,salt_source:config.saltSource});
  }catch(err){
